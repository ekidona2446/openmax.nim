...
async def main():
    ...
    parser.add_argument("--url", default="wss://[::1]:18443/websocket")
    parser.add_argument("--db", default=str(Path(__file__).resolve().parents[1] / "pymax-ws.db"))
    parser.add_argument("--phone", default="+7 999 000 20 01")
    ...
    async with websockets.connect(args.url, ssl=ctx, origin="https://web.max.ru") as ws:
        rpc = WsRpc(ws)
        hello = await rpc.invoke(
            6,
            {
                "deviceId": "pymax-ws-tls-probe",
                "userAgent": {
                    "deviceType": "WEB",
                    "appVersion": "26.14.1",
                    "osVersion": "Linux",
                    ...
                    "locale": "ru",
                    "deviceName": "PyMax WS TLS Probe",
                    "deviceLocale": "ru",
                    "headerUserAgent": "PyMax WS TLS Probe",
                },
            },
        )
        print("SESSION_INIT", hello)

        auth_request = await rpc.invoke(
            17,
            {"phone": args.phone, "type": "START_AUTH", "language": "ru"},
        )
        ...
        if "REGISTER" in auth["payload"].get("tokenAttrs", {}):
            ...
            login_token = auth["payload"]["tokenAttrs"]["LOGIN"]["token"]

        login = await rpc.invoke(19, {"token": login_token, "interactive": True})
        print("LOGIN", login)

        calls = await rpc.invoke(158, {})
        print("CALLS_TOKEN", calls)

    print("ws-tls-probe ok")
